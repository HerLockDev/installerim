...
import sys
import subprocess
from nitro_installer import hata, bilgi, onemli, soru
...
def main():
    ...
    if Sonuc == "2":
        ...
        if soup.title.string == "Create new application":
            ...
            if app_title == '':
                app_title = choice(["nit", "nitr", "nitro", "madelineproto", "telethon", "pyrogram"]) + choice(["", "-", "+", " "]) + choice(["user", "bot", "vue", "jsx", "python", "php"]) + choice([str(randint(10000, 99999)), ""])
            
            app_shortname = soru("APP'ın qısa adı nə olsun? (Avtomatik yaratmaq üçün boş buraxın) \[5-32 xarakter\]: ")
            if app_shortname == '':
                app_shortname = choice(["nit", "nitr", "nitro", "madelineproto", "telethon", "pyrogram"]) + choice(["", "-", "+", " "]) + choice(["user", "bot", "vue", "jsx", "python", "php"]) + choice([str(randint(10000, 99999)), ""])
        ...
        hata("(!) Bilinməyən seçim. Can ciger")
        exit(1)
